...
| 004 real Codex E2E, security/operations hardening | 20% | 15% | Prior diagnostic elaboration is not governed E2E evidence. The historical raw probe is narrowed to `raw_bwrap_unshare_all_loopback_bootstrap_failed`; the OAP parent was independently established as host-direct/unsandboxed, so no nested-outer-sandbox artifact is claimed. The corrected native Codex 0.149.0 `workspace-write`/`:workspace` preflight stopped at no-model `true` with exit status 1 before dependency bytes crossed the boundary; exact dependency `cat`, governed calls, adapter use, and cache-reuse E2E were not run | Native exact dependency read, governance-derived sentinel success, successful ordinary-read lifecycle and cache reuse, actual forced/equivalent long-session compaction, vision-capable E2E, security hardening review, and systemd candidate proof |
