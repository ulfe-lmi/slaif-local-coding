...
| 004 real Codex E2E, security/operations hardening | 20% | 15% | Prior diagnostic elaboration is not governed E2E evidence. The historical raw probe remains narrowly `raw_bwrap_unshare_all_loopback_bootstrap_failed`; the OAP parent was independently established as host-direct/unsandboxed. In 004-o, the ordinary direct `codex exec` B control exited `2` at Codex startup with sanitized `argv_unsupported`/`argument` evidence before JSONL or model/tool events; A, dependency `cat`, adapter/compiler/model calls, and governed/cache E2E were not run. No Local Coding defect was tested and no protected service was changed | Correct global-option invocation, exact ordinary `/usr/bin/true` B+A equivalence, dependency bytes, governance-derived sentinel success, successful ordinary-read lifecycle and cache reuse, actual forced/equivalent long-session compaction, vision-capable E2E, security hardening review, and systemd candidate proof |
