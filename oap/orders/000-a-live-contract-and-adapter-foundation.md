...
> **FINAL STRATEGIC ORDER FOR ATOMIC ACTIVATION.** Live GitHub and protected-host
> facts below were independently reconciled on 2026-08-21 CEST. Publish only
> through the prescribed atomic helper, then signal the coding loop exactly once.
...
- Base branch: `main`
- Starting base SHA: `bcdb3542a74a08219496ef29a09fd13b543e954c`
...
- Remote default branch/SHA: GitHub reports `main` at
  `bcdb3542a74a08219496ef29a09fd13b543e954c`; authenticated `git fetch` and
  local `origin/main` agree.
- Open PRs and objective-000 duplicates: GitHub reports zero open PRs and zero
  open/closed/merged PRs with `OAP 000` in the title.
- Clean/known working tree and bootstrap commit: checkout is clean on local
  `main`, exactly equal to `origin/main`; starting commit subject is
  `Use default Codex for both OAP agents`.
- Current user and host: `janezp@hinton2`, Linux 6.8.0-138-generic x86_64.
- `qwen-serving` unit/service state: user unit
  `qwen-serving-vision.service` is loaded, active/running, disabled, PID 4174,
  started 2026-08-20 23:27:10 CEST. The mutually exclusive user unit
  `qwen-serving.service` is loaded but inactive/dead. There is no system unit
  named `qwen-serving.service`.
- Exact current vLLM process command (no secret argument is present):
  `/synology/homes/janezp/qwen-serving/venv/bin/python /synology/homes/janezp/qwen-serving/venv/bin/vllm serve /synology/homes/janezp/qwen-serving/models/Qwen3.8-27B-W4A16-AutoRound --served-model-name qwen3.8-27b --host 10.8.132.76 --port 18020 --gpu-memory-utilization 0.972 --max-model-len 100000 --max-num-seqs 1 --api-server-count 1 --limit-mm-per-prompt {"image":1} --kv-cache-dtype fp8 --mamba-ssm-cache-dtype float16 --async-scheduling --max-num-batched-tokens 2048 --compilation-config {"max_cudagraph_capture_size":64,"custom_ops":["+rms_norm","+silu_and_mul"]} --reasoning-parser qwen3 --enable-auto-tool-choice --tool-call-parser qwen3_coder --enable-prefix-caching`.
- Actual bind/listeners for 18020/18031 and any unexpected 18021 listener:
  PID 4174 listens only at `10.8.132.76:18020`; no listener exists on 18021 or
  18031.
- Actual upstream base URL/model ID: `http://10.8.132.76:18020/v1`, model
  `qwen3.8-27b`.
- Actual vision-enabled configuration and effective max image count: live
  process has `--limit-mm-per-prompt {"image":1}`, so the effective vLLM image
  cap is one per prompt.
- Pre-existing image-cap proxy: none found in listeners, processes, or user/system
  service units; only the vLLM listener exists on 18020.
- Coding Codex active profile/provider endpoint: wrapper profile
  `oap-coding-low` layers model `gpt-5.6-sol` over the base built-in `openai`
  provider using ChatGPT sign-in; it has no locally configured base URL and is
  not routed through 18020/18031. A separate inactive-for-coding profile
  `qwen38-3090` points to the local vision vLLM at 18020.
- API-key source mechanism/path/env name, value never printed: server start
  script reads protected `/synology/homes/janezp/qwen-serving/api_key.txt`
  (mode 0600) into `VLLM_API_KEY`; the coding-loop environment already contains
  `QWEN3090_API_KEY`, which is the `qwen38-3090` Codex provider's configured
  `env_key`. Values were not printed.
- Authenticated `/health` and `/v1/models` status: both returned HTTP 200 using
  the protected key; `/v1/models` reports exactly `qwen3.8-27b`.
- Port 18031 free: confirmed; no TCP listener.
- Pre-existing protected-config risk: `/synology/homes/janezp/.config/qwen-serving-vision.env`
  is mode 0777. It contains service settings rather than the API key, but its
  permissions are unsafe for a protected launch input. Objective 000 MUST NOT
  mutate this protected file or unit; record its before/after metadata and hash,
  preserve it byte-for-byte, and carry this risk to strategic review for a
